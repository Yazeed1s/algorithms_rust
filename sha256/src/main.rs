...
const W: usize = 32; // -> number of bits in word
const M: usize = 1 << 32;
const FF: usize = M - 1; // -> 0xFFFFFFFF (for performing addition mod 2**32)
...
fn RR(x: usize, b: usize) -> usize {
    (((x >> b) | (x << (W - b))) & FF).try_into().unwrap()
}

fn padding(x: String) -> usize {

    let mdi = x.len() % 64;
    let l = (x.len() << 3).to_be_bytes();
    0
